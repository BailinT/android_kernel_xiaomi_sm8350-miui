			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	if (fts_writeRead(pure_cmd, 3 + DUMMY_FRAMEBUFFER, pure_data, 2 + DUMMY_FRAMEBUFFER) < OK) {
		logError(1,
	...
	res = fts_writeReadU8UX(FTS_CMD_FRAMEBUFFER_R, BITS_16, offset, (u8 *)data_dump_buf1,
			      node_data_size * BYTES_PER_NODE, 0);
	if (res < OK) {
...
static void fts_grip_mode_work(struct work_struct *work)
{
	...
	ret = wait_event_interruptible_timeout(fts_info->wait_queue, !(fts_info->irq_status ||
	fts_info->touch_id || fts_info->sensor_sleep), msecs_to_jiffies(2000));
...
static void fts_resume_work(struct work_struct *work)
{
	...
	queue_delayed_work(info->touch_feature_wq, &info->grip_mode_delay_work,
...
static int fts_probe(struct spi_device *client)
{
	...
	info->fwu_workqueue =
	    alloc_workqueue("fts-fwu-queue1",
			    WQ_UNBOUND | WQ_HIGHPRI | WQ_CPU_INTENSIVE, 1);
...
#ifdef FTS_DEBUG_FS
	info->debugfs = debugfs_create_dir("tp_debug1", NULL);
	if (info->debugfs) {
...
#else
		info->fts_tp_class = class_create(THIS_MODULE, "touch1");
#endif
	info->fts_touch_dev =
	    device_create(info->fts_tp_class, NULL, 0x50, info, "tp_dev1");
	...
	info->tp_lockdown_info_proc =
	    proc_create("tp_lockdown_info_1", 0444, NULL, &fts_lockdown_info_ops);
	info->tp_selftest_proc =
	    proc_create("tp_selftest_1", 0644, NULL, &fts_selftest_ops);
	info->tp_data_dump_proc =
	    proc_create("tp_data_dump_1", 0444, NULL, &fts_datadump_ops);
	info->tp_fw_version_proc =
	    proc_create("tp_fw_version_1", 0444, NULL, &fts_fw_version_ops);
	info->tp_pure_raw_proc =
	    proc_create("tp_pure_raw_1", 0444, NULL, &fts_pure_raw_ops);

#ifdef FTS_XIAOMI_TOUCHFEATURE
	info->touch_feature_wq =
	    alloc_workqueue("fts-touch-feature-1",
			    WQ_UNBOUND | WQ_HIGHPRI | WQ_CPU_INTENSIVE, 1);
	...
	return OK;
ProbeErrorExit_8:
	device_destroy(info->fts_tp_class, 0x50);
	if (info->tp_lockdown_info_proc)
		remove_proc_entry("tp_lockdown_info_1", NULL);
	if (info->tp_selftest_proc)
		remove_proc_entry("tp_selftest_1", NULL);
	if (info->tp_data_dump_proc)
		remove_proc_entry("tp_data_dump_1", NULL);
	if (info->tp_fw_version_proc)
		remove_proc_entry("tp_fw_version_1", NULL);
	if (info->tp_pure_raw_proc)
		remove_proc_entry("tp_pure_raw_1", NULL);
	info->tp_lockdown_info_proc = NULL;
	info->tp_selftest_proc = NULL;
	info->tp_data_dump_proc = NULL;
	info->tp_fw_version_proc = NULL;
	info->tp_pure_raw_proc = NULL;
	/*

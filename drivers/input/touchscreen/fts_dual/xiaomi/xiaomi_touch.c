...
int xiaomitouch_register_modedata(int touchId, struct xiaomi_touch_interface *data)
{
	...
	if (data->ear_sensor_write)
		touch_data->ear_sensor_write = data->ear_sensor_write;
...
struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (pdata->touch_data[1]->palm_sensor_write)
		pdata->touch_data[1]->palm_sensor_write(!!input);
	else {
...
static ssize_t ear_sensor_show(struct device *dev,
struct device_attribute *attr, char *buf)
{
	struct xiaomi_touch_interface *touch_data = NULL;
	if (!touch_pdata) {
	...
	return snprintf(buf, PAGE_SIZE, "%d\n", touch_data->ear_sensor_value);
}

static ssize_t ear_sensor_store(struct device *dev,
struct device_attribute *attr, const char *buf, size_t count)
{
	struct xiaomi_touch_interface *touch_data = NULL;
	unsigned int input;
	...
	pr_info("%s,%d\n", __func__, input);
	if (touch_data->ear_sensor_write)
		touch_data->ear_sensor_write(input);
	touch_data->ear_sensor_switch = !!input;
	sysfs_notify(&xiaomi_touch_dev.dev->kobj, NULL,  "ear_sensor_data");

	return count;
}

static ssize_t ear_sensor_data_store(struct device *dev,
struct device_attribute *attr, const char *buf, size_t count)
{
	struct xiaomi_touch_interface *touch_data = NULL;
	unsigned int input;
	...
	touch_data->ear_sensor_value = !!input;
	sysfs_notify(&xiaomi_touch_dev.dev->kobj, NULL,  "ear_sensor");

	return count;
}

static ssize_t ear_sensor_data_show(struct device *dev,
struct device_attribute *attr, char *buf)
{
	struct xiaomi_touch_interface *touch_data = NULL;
	if (!touch_pdata) {
	...
	return snprintf(buf, PAGE_SIZE, "%d\n", touch_data->ear_sensor_switch);
}

int update_ear_sensor(void)
{
	sysfs_notify(&xiaomi_touch_dev.dev->kobj, NULL,
		     "ear_sensor_data");
...
EXPORT_SYMBOL_GPL(update_ear_sensor);
...
static DEVICE_ATTR(ear_sensor, (S_IRUGO | S_IWUSR | S_IWGRP),
		   ear_sensor_show, ear_sensor_store);

static DEVICE_ATTR(ear_sensor_data, (S_IRUGO | S_IWUSR | S_IWGRP),
		   ear_sensor_data_show, ear_sensor_data_store);
...
static struct attribute *touch_attr_group[] = {
	...
	&dev_attr_ear_sensor.attr,
	&dev_attr_ear_sensor_data.attr,
